import { LETTER_MAPPINGS } from './letterMappingsComplete';

export type LetterToIPAMapping = {
  letters: string;      // The letter(s) in the text
  positions: number[];  // Character positions in the text
  ipa: string;         // The IPA symbol(s) these letters produce
};
...
export type TwisterItem = {
  ...
  text: string;
  ipaLines?: string[];
  letterMappings?: LetterToIPAMapping[][]; // Per line, per word/segment mapping
  focus: string;
...
export const TONGUE_TWISTERS: TwisterItem[] = [
  {
    ...
    letterMappings: LETTER_MAPPINGS['she-sells'],
    focus: '/s/ vs /ʃ/',
    level: 'Beginner',
  },
  {
  ...
  },
  {
    ...
    letterMappings: LETTER_MAPPINGS['i-thought-a-thought'],
    focus: '/θ/ and /t/',
    level: 'Intermediate',
  },
  {
