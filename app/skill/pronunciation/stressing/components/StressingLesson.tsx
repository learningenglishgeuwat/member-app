...
const SENTENCE_STRESS_EXAMPLES = [
  {
    ...
    ipa: '/aɪ ˈwɑn.tɪd ə rɛd pɛn, nɑt ə blu wʌn/',
  ...
  },
  {
    ...
    ipa: '/ʃi kæn swɪm, bʌt ʃi ˈkæn.ɑt daɪv/',
  ...
  },
  {
    ...
    ipa: '/aɪ dɪd nɑt seɪ hi stoʊl ðə ˈmʌn.i/',
...
const CONTENT_FUNCTION_EXAMPLES = [
  {
    ...
    ipa: '/ðə nu ˈprɑdʒ.ɛkt nidz klɪr ɡoʊlz/',
  },
  {
...
const WORD_STRESS_EVALUATION_PROMPT =
...
const SENTENCE_STRESS_EVALUATION_PROMPT =
  "Saya telah mengunggah rekaman audio. Saya ingin Anda bertindak sebagai penilai aksen bahasa Inggris profesional. 1. Transkripsikan kalimat yang saya ucapkan. 2. Analisis pengucapan dengan fokus pada American Accent (General American), terutama sentence stress (kata mana yang ditekan), ritme stress-timed, serta kontras content words vs function words (weak forms) agar kalimat terdengar natural. 3. Format output: sajikan hasil analisis dalam bentuk tabel dengan tiga kolom: - Kolom 1: Kalimat yang diucapkan. - Kolom 2: Status kualitatif ('🟢 Sangat bagus 🔵Bagus', '🟡 Perlu Sedikit Perbaikan', atau '🔴 Perlu Perbaikan'). - Kolom 3: Umpan balik spesifik yang menjelaskan bagian stress/ritme mana yang perlu diperbaiki.";

export type StressingLessonVariant = 'word' | 'sentence';
...
export default function StressingLesson({ variant }: { variant: StressingLessonVariant }) {
  const isWord = variant === 'word';
  const topicTitle = isWord ? 'Word Stress' : 'Sentence Stress';
  const topicSubtitle = isWord
    ? 'Latihan tekanan suku kata agar kata terdengar natural dan jelas.'
    : 'Latihan tekanan kalimat agar ritme dan fokus makna lebih natural.';
  const progressId = isWord ? 'stressingWord' : 'stressingSentence';
  const sectionStateKey = isWord
    ? 'stressing-word-open-sections-v1'
  ...
  const [showHighlightBySection, setShowHighlightBySection] = useState<Record<string, boolean>>({
    dasarSukuKata: true,
    aturanCepat: true,
    tekananKata: true,
    kontrasNounVerb: true,
  ...
  const handleSaveProgress = useCallback(
    async (percentage: number) => {
      ...
      pronunciationProgress[progressId] = percentage;
    ...
    [calcPronunciationAverage, progressId],
  );
  ...
  const handleUnsaveProgress = useCallback(async () => {
    ...
    delete pronunciationProgress.stressing;
    delete pronunciationProgress[progressId];
    window.localStorage.setItem(PRONUNCIATION_PROGRESS_KEY, JSON.stringify(pronunciationProgress));
  ...
  function toggleHighlightBySection(sectionKey: string) {
    setShowHighlightBySection((prev) => ({
      ...prev,
      [sectionKey]: !prev[sectionKey],
    }));
  }
  ...
  useEffect(() => {
    ...
    try {
      const saved = window.localStorage.getItem(sectionStateKey);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const legacy = pronunciationProgress.stressing;
      const hasNewKeys =
        typeof pronunciationProgress.stressingWord === 'number' ||
        typeof pronunciationProgress.stressingSentence === 'number';

      if (typeof legacy === 'number' && Number.isFinite(legacy) && !hasNewKeys) {
        pronunciationProgress.stressingWord = legacy;
        pronunciationProgress.stressingSentence = legacy;
        delete pronunciationProgress.stressing;
        window.localStorage.setItem(PRONUNCIATION_PROGRESS_KEY, JSON.stringify(pronunciationProgress));
      }
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const value = currentProgress[progressId];
      setIsProgressSaved(typeof value === 'number' && Number.isFinite(value) && value > 0);
    } catch {
      setIsProgressSaved(false);
    }
  }, [progressId]);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(sectionStateKey, JSON.stringify(openSections));
    } catch {
      // Ignore storage write failures.
    }
  }, [openSections, sectionStateKey, sectionStateReady]);
  ...
  return (
    <div className="pronunciation-layout pronunciation-theme pronunciation-theme--stressing stress-page">
      ...
      <main className="stress-shell">
        ...
        <section
          id="dasarSukuKata"
          className={`stress-block ${showHighlightBySection.dasarSukuKata ? '' : 'stress-highlight-off'}`}
        >
        ...
        <section
          id="aturanCepat"
          className={`stress-block ${showHighlightBySection.aturanCepat ? '' : 'stress-highlight-off'}`}
        >
        ...
        <section
          id="tekananKata"
          className={`stress-block ${showHighlightBySection.tekananKata ? '' : 'stress-highlight-off'}`}
        >
        ...
        <section
          id="kontrasNounVerb"
          className={`stress-block ${showHighlightBySection.kontrasNounVerb ? '' : 'stress-highlight-off'}`}
        >
        ...
        <section
          id="bankKata"
          className={`stress-block ${showHighlightBySection.bankKata ? '' : 'stress-highlight-off'}`}
        >
        ...
        <section
          id="tekananKalimat"
          className={`stress-block ${showHighlightBySection.tekananKalimat ? '' : 'stress-highlight-off'}`}
        >
                  ...
                  return (
                    ...
                    >
                      <div className="stress-list-item-main">
                        ...
                        {showIpaBySection.tekananKalimat ? (
        ...
        <section
          id="kataKontenFungsi"
          className={`stress-block ${showHighlightBySection.kataKontenFungsi ? '' : 'stress-highlight-off'}`}
        >
                ...
                {CONTENT_FUNCTION_EXAMPLES.map((item, index) => {
                  ...
                  <div className="stress-rule-examples">
                    <div className={`stress-rule-example-item ${isSpeaking ? 'is-speaking' : ''}`}>
                      ...
                      {showIpaBySection.kataKontenFungsi ? (
                        <p className="stress-ipa stress-ipa--inline">{item.ipa}</p>
                      ) : null}
                      <button
                        type="button"
        ...
        <section
          id="practice"
          className={`stress-block ${showHighlightBySection.practice ? '' : 'stress-highlight-off'}`}
        >
          ...
          {openSections.practice && (
            <>
              ...
              <p className="stress-text font-sans">
                ...
                <br />
                {isWord
                  ? 'Bacakan: 4 kata dengan posisi stress berbeda (pertama sampai keempat).'
                  : 'Bacakan: 3 kalimat latihan dengan sentence stress yang jelas.'}
              </p>
          ...
          {openSections.prompt && (
            <div className="stress-prompt-card">
              <div className="stress-prompt-header">
                <p className="stress-prompt-title">
                  {isWord ? 'Prompt Penilaian Word Stress' : 'Prompt Penilaian Sentence Stress'}
                </p>
                <button
                  type="button"
