...
import {
  D_ED_APPLICATION_SCENES,
  MICRO_STEP_DURATION_MS,
  TOTAL_STEPS_PER_SCENE,
} from './d-ed-simulation-data';

type DEdSimulationPlayerProps = {
...
function renderHighlightedSuffix(text: string, suffix: string, enabled: boolean) {
  ...
  return (
    <>
      {prefix}
      <span className="d-ed-process-highlight">{suffix}</span>
      {rest}
    </>
  );
}
...
function renderHighlightedTail(ipa: string, tail: string, enabled: boolean) {
  ...
  return (
    <>
      {prefix}
      <span className="d-ed-process-highlight">{tail}</span>
      {rest}
    </>
  );
}

export default function DEdSimulationPlayer({
  variant,
  autoStart = true,
}: DEdSimulationPlayerProps) {
  ...
  const activeScene = D_ED_APPLICATION_SCENES[activeSceneIndex];
  ...
  const processProgress = useMemo(
    () =>
      ((activeSceneIndex * TOTAL_STEPS_PER_SCENE + (processStepIndex + 1)) /
        (D_ED_APPLICATION_SCENES.length * TOTAL_STEPS_PER_SCENE)) *
      100,
  ...
  const handleProcessPlayPause = () => {
    ...
    if (
      activeSceneIndex >= D_ED_APPLICATION_SCENES.length - 1 &&
  ...
  const handleProcessNext = () => {
    setIsPlayingProcess(false);
    setActiveSceneIndex((prev) => Math.min(D_ED_APPLICATION_SCENES.length - 1, prev + 1));
    setProcessStepIndex(0);
  };
  ...
  useEffect(() => {
    ...
    if (processStepIndex !== 0) return;

    const sceneKey = `${activeSceneIndex}-${activeScene.beforeWord}`;
    if (autoBeforeTtsSceneRef.current === sceneKey) return;

    autoBeforeTtsSceneRef.current = sceneKey;
    void speakWithBestEnglishVoice(activeScene.beforeWord);
  }, [activeScene.beforeWord, activeSceneIndex, isPlayingProcess, processStepIndex]);

  useEffect(() => {
    if (!isPlayingProcess) return;
    if (processStepIndex !== LAST_STEP_INDEX) return;

    const sceneKey = `${activeSceneIndex}-${activeScene.afterWord}`;
    if (autoAfterTtsSceneRef.current === sceneKey) return;
  ...
  useEffect(() => {
    ...
    const lastScene = D_ED_APPLICATION_SCENES.length - 1;
    const timerId = window.setTimeout(() => {
      setProcessStepIndex((prevStep) => {
        if (prevStep < LAST_STEP_INDEX) return (prevStep + 1) as 0 | 1 | 2 | 3;
  ...
  }, [activeSceneIndex, isPlayingProcess, processStepIndex]);

  return (
    <div className={`d-ed-process-player d-ed-process-player--${variant}`}>
      <div className="d-ed-process-header">
        <h3>{activeScene.title}</h3>
        <span>
          Scene {activeSceneIndex + 1} / {D_ED_APPLICATION_SCENES.length}
        </span>
      </div>

      <div className="d-ed-process-screen">
        <div className="d-ed-process-preview-stack">
          <article className="d-ed-process-focus-card">
            <p className="d-ed-process-card-label">Current Output Sound</p>
            <strong>{activeScene.resultSound}</strong>
          </article>

          <article className={`d-ed-process-word-card ${wordCardStateClass}`}>
            <div className="d-ed-process-card-head">
              <p className="d-ed-process-card-label">Word Card</p>
              <button
                type="button"
            ...
            <p className="d-ed-process-card-value">
              {renderHighlightedSuffix(displayedWord, displayedWordTail, showHighlight)}
            </p>
          </article>

          {showIpaCard ? (
            <article className={`d-ed-process-ipa-card ${ipaCardStateClass}`}>
              <p className="d-ed-process-card-label">IPA Card</p>
              <p className="d-ed-process-card-value">
          ...
          <article className="d-ed-process-rule-card">
            <p className="d-ed-process-description">{activeScene.description}</p>
            <p className="d-ed-process-code-line is-active">{activeScene.codeLine}</p>
            <p className="d-ed-process-rule-note">{activeScene.ruleHint}</p>
          </article>
        </div>
      </div>

      <div
        className="d-ed-process-progress"
        role="progressbar"
        aria-valuemin={0}
        aria-valuemax={100}
        aria-valuenow={Math.round(processProgress)}
      >
        <div className="d-ed-process-progress-fill" style={{ width: `${processProgress}%` }} />
      </div>

      <div className="d-ed-process-controls">
        <button
          type="button"
          className="d-ed-process-btn"
        ...
        <button
          type="button"
          className="d-ed-process-btn d-ed-process-btn-primary"
        ...
        <button
          ...
          disabled={activeSceneIndex === D_ED_APPLICATION_SCENES.length - 1}
      ...
      <div className="d-ed-process-scene-cards">
        {D_ED_APPLICATION_SCENES.map((scene, index) => (
          <button
            key={scene.id}
            type="button"
            className={`d-ed-process-scene-card ${index === activeSceneIndex ? 'is-active' : ''}`}
            onClick={() => handleSelectScene(index)}
            aria-label={`Go to scene ${index + 1}: ${scene.title}`}
          >
            <span className="d-ed-process-scene-index">Scene {index + 1}</span>
            <strong className="d-ed-process-scene-title">{scene.title}</strong>
            <span className="d-ed-process-scene-sound">{scene.resultSound}</span>
          </button>
        ))}
      </div>
    </div>
