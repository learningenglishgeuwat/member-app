...
const AMERICAN_T_SILENT_PROGRESS_ID = 'americanTSilent';
...
export default function SilentTMiddlePage() {
  const [isProgressSaved, setIsProgressSaved] = useState(() => {
    ...
    try {
      ...
      const assessmentKey = 'Silent /t/'.toLowerCase().replace(/\s+/g, '_');
      ...
      return (
        (typeof currentProgress[AMERICAN_T_SILENT_PROGRESS_ID] === 'number' &&
          currentProgress[AMERICAN_T_SILENT_PROGRESS_ID] > 0) ||
        hasSavedAssessment
      );
    } catch {
      return false;
  ...
  const handleSaveProgress = useCallback(
    async (percentage: number) => {
      ...
      pronunciationProgress[AMERICAN_T_SILENT_PROGRESS_ID] = percentage;
  ...
  const handleUnsaveProgress = useCallback(async () => {
    ...
    delete pronunciationProgress.americanT;
    delete pronunciationProgress[AMERICAN_T_SILENT_PROGRESS_ID];
    window.localStorage.setItem(PRONUNCIATION_PROGRESS_KEY, JSON.stringify(pronunciationProgress));
      ...
      <ControlCenter>
        <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            <button onClick={() => isPlayingWordBankAll ? stopAllPlayAll() : playAllWordBank()} className="w-full bg-[#1a1f24] border border-white/10 text-white/80 px-2 py-1.5 sm:px-4 sm:py-3 font-mono text-[8px] sm:text-xs uppercase rounded-lg sm:rounded-xl flex items-center justify-between hover:bg-cyan-900/20 hover:border-cyan-500/30 transition-all group mb-2 sm:mb-3">
              <span className="tracking-widest font-bold">PLAY 50 WORDS</span>
              <Play className={`w-3 h-3 sm:w-4 sm:h-4 transition-colors ${isPlayingWordBankAll ? 'fill-cyan-400 stroke-cyan-400 text-cyan-400' : 'fill-transparent stroke-current group-hover:fill-cyan-400 group-hover:stroke-cyan-400 group-hover:text-cyan-400'}`} />
            </button>
