...
interface RecordingControlsButtonMP3Props {
  downloadFileName?: string;
  className?: string;
  showHelp?: boolean;
}

const RecordingControlsButtonMP3: React.FC<RecordingControlsButtonMP3Props> = ({
  ...
  // Fungsi untuk konversi WAV ke MP3 menggunakan lamejs
  const convertToMp3 = async (wavBlob: Blob): Promise<Blob> => {
    setIsEncoding(true);
    try {
      // Baca WAV blob sebagai ArrayBuffer
      const arrayBuffer = await wavBlob.arrayBuffer();
      
      // Buat AudioContext untuk decode audio
      ...
      const samples = audioBuffer.getChannelData(0); // Mono atau channel pertama
      
      // Konversi Float32Array ke Int16Array (PCM 16-bit)
      ...
      // Setup MP3 encoder
      const mp3encoder = new lamejs.Mp3Encoder(1, sampleRate, 128); // mono, sample rate, bitrate 128kbps
      ...
      // Gabungkan semua chunks menjadi single blob dengan MIME type yang benar
      ...
      setIsEncoding(false);
      setErrorMessage('Gagal convert ke MP3. Menggunakan format asli.');
      return wavBlob; // Fallback ke WAV jika gagal
    }
  };
          ...
          <div className="relative bg-[#050a10]/95 backdrop-blur-xl border border-cyber-cyan/40 rounded-2xl p-3 md:p-4 shadow-[0_0_50px_rgba(190,41,236,0.2)] overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-3 md:gap-6 overflow-x-auto scrollbar-hide">
              <div className="flex flex-col items-center gap-2 flex-shrink-0">
                ...
                <span className={`text-[8px] md:text-[10px] font-mono font-bold tracking-widest ${isRecording ? 'text-red-500' : isEncoding ? 'text-yellow-500' : 'text-cyber-cyan/70'}`}>
                  {isRecording ? 'REC_ON' : isEncoding ? 'ENCODING' : 'INIT_REC'}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 md:gap-3 flex-1 justify-center">
                <button 
                  ...
                  disabled={!recordedAudio || isRecording || isEncoding}
                ...
                <button 
                  ...
                  disabled={!recordedAudio || isEncoding}
            ...
            {isEncoding && (
              <div className="mt-3 text-center text-[10px] md:text-xs text-yellow-400 flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-xl font-bold text-white">Instruksi Perekaman MP3</h3>
            </div>
            ...
            <div className="space-y-3 text-gray-300">
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="flex-1">Tunggu proses <span className="text-yellow-400 font-semibold">encoding ke MP3</span> selesai</p>
              </div>
              <div className="flex items-start gap-3">
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="flex-1">Klik tombol <span className="text-cyber-pink font-semibold">DOWNLOAD</span> untuk menyimpan audio MP3</p>
              </div>
              <div className="flex items-start gap-3">
              ...
              <div className="flex items-start gap-3">
                <span className="text-cyber-cyan font-bold w-5 text-right">8.</span>
            ...
            <div className="mt-4 p-3 bg-cyber-cyan/10 border border-cyber-cyan/30 rounded-lg">
              <p className="text-sm text-cyber-cyan mb-2">
                <strong>Format Audio:</strong> MP3 (128 kbps, mono)
              </p>
              <p className="text-sm text-cyber-cyan">
                <strong>Tip:</strong> Pastikan mikrofon Anda bekerja dengan baik dan berbicara dengan volume yang cukup. Audio akan otomatis dikonversi ke MP3 setelah perekaman.
              </p>
            </div>
          </div>
        </div>
...
export default RecordingControlsButtonMP3;
